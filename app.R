...
DF <- read.csv("macs.csv", stringsAsFactors = FALSE)
# facs <- c(2,8,9,10,11)
# can also use name of index with '$'

#The user interface
header <- dashboardHeader(title = "Apple refurbished",
                          titleWidth = 500)
sidebar <- dashboardSidebar(disable = TRUE)
body <- dashboardBody(
  fluidRow(stylesheet,
    column(width=2,
           selectInput("factor1", "Factor 1", colnames(DF[,3:7])),
           selectInput("factor2", "Factor 2", colnames(DF[,3:7])),
           selectInput("num1", "Numeric 1", colnames(DF[,1:2])),
           selectInput("num2", "Numeric 2", colnames(DF[,1:2])),
